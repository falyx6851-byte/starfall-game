/* M03 asset-review host. Uses v0.1.3 GLB adapter, Animator and rendering passes.
...
(async()=>{
 ...
 $('export').onclick=()=>{let bytes=Uint8Array.from(atob(HERO_DATA.glb),x=>x.charCodeAt(0));let url=URL.createObjectURL(new Blob([bytes],{type:'model/gltf-binary'}));let a=document.createElement('a');a.href=url;a.download='Stargazer_M03.glb';a.click();setTimeout(()=>URL.revokeObjectURL(url),5000);};
 ...
 window.__HERO_SAMPLE__={
  ...
  draw,report(){return{version:'M03',modelSHA256:HERO_DATA.hash,clip:state.clip,time:state.time,view:state.view,gray:state.gray,eye,vp:Array.from(vp),frame,metadata:asset.metadata,renderer:r.gl.getParameter(r.gl.RENDERER),width:r.width,height:r.height,glError:r.gl.getError(),shadowSize:r.shadowSize,bonesFinite:Array.from(anim.bones).every(Number.isFinite)};},
