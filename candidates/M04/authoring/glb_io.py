...
def replace_mesh(base_path,out_path,positions,normals,joints,weights,uv,faces,metadata):
 ...
 new['asset']['generator']='Stargazer M04 / H01 base / design-space shoulder envelope'
 new['extras']={**j.get('extras',{}),'sampleVersion':'M04','status':'plain coverage candidate, not visually approved or installed in game','scope':'only mesh 08 Asymmetric shoulder mantle','sourceH01SHA256':hashlib.sha256(Path(base_path).read_bytes()).hexdigest(),'checkedClips':[], 'animationReview':'refer to M04 reports; inherited H01 claims do not apply', 'mantle':metadata}
